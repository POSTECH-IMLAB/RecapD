r"""
A utility module to easily load common capD models (optionally with pretrained
weights) using a single line of code.

Get our full best performing capD model (with pretrained weights as):

>>> import capD.model_zoo as mz
...
from capD.factories import PretrainingModelFactory
...
def get(config_path, pretrained: bool = False):
    ...
    _pkg_config_path = pkg_resources.resource_filename(
        "capD.model_zoo", os.path.join("configs", config_path)
    )
    ...
    if pretrained:
        ...
        checkpoint_path = download(
            checkpoint_url,
            dir=os.path.expanduser("~/.torch/capD_cache"),
            filename=os.path.basename(config_path).replace(".yaml", ".pth")
        )
